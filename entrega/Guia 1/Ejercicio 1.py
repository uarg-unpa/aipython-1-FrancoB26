print ("Las máquinas me sorprenden con mucha frecuencia")

print("")

print("Mensaje")

print("23")
print(23)
#El 23 fue mostrado de ambas maneras, con comillas o sin comillas

print('Una computadora puede ser llamada "inteligente" si logra engañar a una persona haciéndole creer que es un humano.')

print("Franco")
print("")
print("Bahamonde")
print("")
print("21 años")

print("Franco","Bahamonde","21 años", sep="_")

print("Calle: Romualdo D Andrea","\tNumero: 518","\tCodigo Postal: 9400")

print("Calle: Romualdo D Andrea")
print("Numero: 518")
print("Codigo Postal: 9400")

print("Feliz\n\tPrimavera\n\t\t\t2024")

print("Solo podemos ver poco del futuro,", end=" ")
print("pero lo suficiente para darnos cuenta de que hay mucho que hacer.")

print("    *")
print("   * *")
print("  *   *")
print(" *     *")
print("***   ***")
print("  *   *")
print("  *   *")
print("  *****")







